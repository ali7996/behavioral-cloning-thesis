...
component of a 2023 Master's thesis on this task~\mbox{\citep{elgemei2023behavioral}} and
...
input~\mbox{\citep{pomerleau1989alvinn,bojarski2016end}}. It requires no hand-engineered
...
thesis~\mbox{\citep{elgemei2023behavioral}} that implemented and compared three
...
input~\mbox{\citep{pomerleau1989alvinn}}. The approach was revived at scale by NVIDIA's
...
supervision for them~\mbox{\citep{bojarski2016end}}. The architecture used in this paper is
...
Plain behavioral cloning is known to be brittle. \mbox{\citet{codevilla2018end}} show that
...
command to fix this. \mbox{\citet{codevilla2019exploring}} further document that behavior
...
Long Short-Term Memory networks~\mbox{\citep{hochreiter1997long}} are the standard tool for
...
optimizer~\mbox{\citep{kingma2015adam}} ($\text{lr}=10^{-3}$) and a mean-squared-error
loss.

\paragraph{Convolutional baseline (single frame).}
This is the DAVE-2 network~\mbox{\citep{bojarski2016end}}: five convolutional layers
...
LSTM~\mbox{\citep{hochreiter1997long}} layers of 128 units, then a dense head. Flattening
...
Thesis~\mbox{\citep{elgemei2023behavioral}}, reported & --- & 0.300 & 0.408 & --- \\
\bottomrule
\end{tabular}
\end{table}
